import { Double, Quadouble } from ".";
export type LongMonth = 'Janurary' | 'February' | 'March' | 'April' | 'May' | 'June' | 'July' | 'August' | 'September' | 'October' | 'November' | 'December';
export type ShortMonth = 'Jan' | 'Feb' | 'Mar' | 'Apr' | 'May' | 'Jun' | 'Jul' | 'Aug' | 'Sep' | 'Oct' | 'Nov' | 'Dec';
export type Month = LongMonth | ShortMonth;
type Ends<Date extends number> = Date extends 1 ? 'st' : Date extends 2 ? 'nd' : Date extends 3 ? 'rd' : 'th';
export type ShortDay = 'Mon' | 'Tue' | 'Wed' | 'Thu' | 'Fri' | 'Sat' | 'Sun';
export type LongDay = `${'Mon' | 'Tues' | 'Wednes' | 'Thurs' | 'Fri' | 'Satur' | 'Sun'}day`;
export type Day = ShortDay | LongDay;
export declare const weekdays: Day[];
export declare const weekend: Day[];
type ShortFormat<MM extends Month = Month, dd extends number = number> = `${dd}${Ends<dd>} ${MM}, ${Quadouble}`;
type LongFormat<WeekDay extends Day = Day, dd extends number = number, MM extends Month = Month> = `${WeekDay}, ${Ends<dd>} ${MM} ${Quadouble}`;
export type DateFormats<Seperator extends string = '/'> = `${Double}${Seperator}${Double}${Seperator}${Double | Quadouble}` | ShortFormat | LongFormat;
export default DateFormats;
