...
export default function TimestampPicker({ type = '24h', format = '$hh24:$mm', onChange }: Props) {
  ...
  return (
    <div className="timestamp-picker">
      <input className="timestamp-picker__hour" type="number" onChange={e => setHour(e.target.value)} value={hour} />
      <input className="timestamp-picker__minute" type="number" onChange={e => setMinute(doubleDigit(parseInt(e.target.value)))} value={minute} />
